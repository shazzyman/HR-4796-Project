...
from 3comp_disk_model import disk_model_fun_new
...
def lnprior_new(param_list):
    ...
    if 2. < amin < 10. and 30. < mexp < 34 and 0. <= f_1 <= 1. and 0. <= f_2 <= 1. \
            and 0. <= f_3 <= 1. \
            and 0.99 < f_1 + f_2 + f_3 < 1.01 and -1.0 < lnf < 1.0:
        return 0.0
    return -np.inf
...
def lnlike_new(param_list):
    amin, mexp, f_1, f_2, f_3, lnf = param_list
    ld, ldp, lx, ly, fluxmodel_trash = disk_model_fun_new(wl, amin, amax, aexp, mexp, f_1, f_2, f_3,
...
global wl, amax, m_1, m_2, m_3, projected_separation_pts_au, fluxmeasured, fluxerr, aexp
...
# PUT THIS SECTION ON INTO A CODE THAT YOU CAN FUNCTIONALIZE TO MAKE MORE USERFRIENFLY 
# # Astrosilicate
