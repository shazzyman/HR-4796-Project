#!/usr/bin/env python
"""
This Python script is designed to perform an MCMC (Markov Chain Monte Carlo) analysis of a 10 paramerter dust disk model, leveraging MPI parallelization for 
...
def lnprior(param_list):
    ...
    if 0.01 < amin < 4. and -5. < aexp < -1. and 29.8 < mexp < 31.4 and 0. <= f_asi <= 1. and 0. <= f_ac <= 1. \
            and 0. <= f_wi <= 1. and 0. <= f_th <= 1. and 0. <= f_tr <= 1. and 0. <= f_fe <= 1. \
            and 0.99 < f_asi + f_ac + f_wi + f_th + f_tr + f_fe < 1.01 and -1.0 < lnf < 1.0:
        return 0.0
    return -np.inf
...
def lnlike(param_list):
    ...
    return -0.5*(np.sum((fluxmeasured-fluxmodel)**2*inv_sigma2 - np.log(inv_sigma2))), fluxmodel
...
# Set up MCMC
coords = np.random.randn(100, 10) # This is (number of chains, number of variables)
...
pos_min = np.array([0.01, -5., 29.8, 0., 0., 0., 0., 0., 0., -0.1])  # *Posterior min and max*
pos_max = np.array([4., -1., 31.4, 1., 1., 1., 1., 1., 1., 0.1])
...
pos = np.array(pos)
filename = "shaz_emcee_Agl_6C_100X10X10000_07/22.h5"   # *File name to save posteriors* Can be used to continue calculation or make corner plots
...
pos = np.array(pos)
backend = emcee.backends.HDFBackend('shaz_emcee_Agl_6C_100X10X10000_07/22.h5')

# Save input parameters to plot modeled spectra later
initial = h5py.File('shaz_MCMC_Agl_6C_100X10X10000_07/22.hdf5', 'w')
