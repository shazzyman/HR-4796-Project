...
def lnprior_new(param_list):
    amin, aexp, mexp, f_asi, f_ac, f_wi, lnf = param_list
    if 2. < amin < 10. and -4 <= aexp <= -3 and 30. < mexp < 34 and 0. <= f_asi <= 1. and 0. <= f_ac <= 1. \
            and 0. <= f_wi <= 1. \
...
global wl, amax, m_asi, m_ac, m_wi, projected_separation_pts_au, fluxmeasured, fluxerr
...
# Set up MCMC
coords = np.random.randn(60, 7) # This is (number of chains, number of variables)
...
pos_min = np.array([2, -4, 30., 0.6, 0., 0., -0.1])  # *Posterior min and max*
pos_max = np.array([10., -3, 34, 1., 1., 1., 0.1])
...
for i in range(nwalkers):
    ...
    mfsi_i = pos_min[3] + psize[3] * np.random.rand()
    f_other = 1 - mfsi_i
    f_ac = 0.5*f_other
    f_wi = 0.5*f_other
    ...
    #                                                     # that sums to 1
    lnf_i = pos_min[6] + psize[6] * np.random.rand()
    pos[i] = np.array([amin_i, aexp_i, mexp_i, mfsi_i, f_ac, f_wi, lnf_i])
pos = np.array(pos)
filename = "7p_shaz_emcee_Agl_6C_60X7X500_Astro_silicate_Draine_2003.h5"   # *File name to save posteriors* Can be used to continue calculation or make corner plots
...
pos = np.array(pos)
backend = emcee.backends.HDFBackend('7p_shaz_emcee_Agl_6C_60X7X500_Astro_silicate_Draine_2003.h5')

# Save input parameters to plot modeled spectra later
initial = h5py.File('7p_shaz_MCMC_Agl_6C_60X7X500_Astro_silicate_Draine_2003.hdf5', 'w')
