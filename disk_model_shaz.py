    ...
    for xi in range(0, npix):
        for yi in range(0, npix):
            for zi in range(0, zpix):
                pgi = pg[:, :, np.int64(np.round((180 / np.pi) * phang[xi, yi, zi]))]
...
def disk_model_fun(wl, amin, amax, aexp, mexp, f_asi, f_ac, f_wi, f_th, f_tr, f_fe, m_asi, m_ac, m_wi, m_th, m_tr, m_fe,
    ...
    for npspts_i in range(0, npspts):
        start_pts = np.int64(npix / 2 + projected_separation_pts_au[npspts_i]/dpar.pxs - np.floor(dpar.nspx))
        stop_pts = np.int64(start_pts + dpar.nspx)
        fluxmodel = np.append(fluxmodel, 'aperture_sum')
