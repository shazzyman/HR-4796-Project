```python
"""
This Python script analyzes the spectral 6 properties of debris disks around stars, focusing on 
...
def lnlike_temp_new(param_list):
    amin, mexp, f_asi, f_ac, f_wi, lnf = param_list
    ld, ldp, lx, ly, fluxmodel_trash = disk_model_fun_new(wl, amin, amax, aexp, mexp, f_asi, f_ac, f_wi,
                                                    m_asi, m_ac, m_wi, pixsc, npix, zpix, sigma_up, sigma_y, sigma_d,
                                                    phang, scatt_type, projected_separation_pts_au)
...
def lnprior_new(param_list):
    amin, mexp, f_asi, f_ac, f_wi, lnf = param_list
    if 0.01 < amin < 4. and 29.8 < mexp < 31.4 and 0. <= f_asi <= 1. and 0. <= f_ac <= 1. \
            and 0. <= f_wi <= 1. \
...
global wl, amax, aexp, m_asi, m_ac, m_wi, m_th, m_tr, m_fe, projected_separation_pts_au, fluxmeasured, fluxerr
...
#Orthopyro
nk_file_asi = './optical_constants/brug_olivineIP80_orthopyr_por90.txt'
nkdat_asi = np.loadtxt(nk_file_asi, unpack=True)
wlnk_asi = np.flipud(nkdat_asi[0, :])
nf_asi = np.flipud(1 + nkdat_asi[1, :])
kf_asi = np.flipud(nkdat_asi[2, :])
...
#=============================================================================
mexp_test_vals = np.array([31.7, 31.9, 32.1, 32.3, 32.5, 32.7])
mexp_test_vals = np.array([31.82])
n_mexp = mexp_test_vals.size
mexp_loop = np.zeros(n_mexp)

for i in range(0, n_mexp): 
    param_list = amin, mexp_test_vals[i], mfsi, mfac, mfwi, lnf

    

    mexp_loop[i], fluxmodel = chi_square(param_list)
    #mexp_loop[i], fluxmodel = chi_square(param_list)
